...
export function mergeSafeguards(state, safeguards) {
  ...
  safeguards.forEach(generated => {
    ...
    if (!existing) return;
    // Upgrade persisted cards with new response metadata without replacing
    // the surveyor's wording, checked state or section choice.
    ...
    if (existing.includeInNotes === undefined) existing.includeInNotes = generated.includeInNotes;
  });
