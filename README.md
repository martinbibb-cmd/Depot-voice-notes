...
- Cloudflare Worker proxy to OpenAI, Gemini, or Anthropic for AI processing
- Automatic fallback to Gemini and Anthropic Claude if OpenAI fails
...
3. **Configure and deploy the Worker**
   ...
   wrangler secret put GEMINI_API_KEY      # Optional: for Gemini support
   wrangler secret put ANTHROPIC_API_KEY  # Optional: for Anthropic support
   wrangler deploy
   ```
   Update `ALLOWED_ORIGIN` (and optional additional origins) plus `PUBLIC_KEY_JWK_X` in `wrangler.toml`.

   **Note:** You can configure one or more API keys:
   - `OPENAI_API_KEY` only: Uses OpenAI exclusively
   - `GEMINI_API_KEY` only: Uses Google Gemini exclusively
   - `ANTHROPIC_API_KEY` only: Uses Anthropic Claude exclusively
   - Multiple keys: Uses OpenAI by default, falls back to Gemini, then Anthropic if earlier providers fail
